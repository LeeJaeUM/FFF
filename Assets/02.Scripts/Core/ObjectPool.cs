...
// where T : MonoBehaviour // T 타입은 MonoBehaviour이거나 MonoBehaviour를 상속받는 클래스만 가능하다.
public class ObjectPool<T> : MonoBehaviour where T : RecycleObject
{
    ...
    /// <summary>
    /// 풀의 크기. 처음에 생성하는 오브젝트의 갯수. 모든 갯수는 2^n로 잡는 것이 좋다.
    /// </summary>
    ...
    public void Initialized()
    {
        if (pool == null)
        {
            pool = new T[poolSize];                  // 배열의 크기 만큼 new
    ...
    void GenerateObjects(int start, int end, T[] results)
    {
        ...
        {
            GameObject obj = Instantiate(originalPrefab, transform);    // 프리팹 생성하기
